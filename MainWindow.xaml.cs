...
namespace WinFastGUI
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            LoadSavedLanguage();
            LogMessageToMainWindow("WinFast Optimizasyon Aracı başlatıldı.");
        }
        ...
        {
            string savedCulture = Properties.Settings.Default.Language ?? "tr-TR";
            Console.WriteLine($"Settings'ten okunan dil: {savedCulture}");
            ...
            Thread.CurrentThread.CurrentUICulture = CultureInfo.GetCultureInfo(savedCulture);
        ...
        {
            if (!IsLoaded || LanguageComboBox.SelectedItem is not ComboBoxItem selectedItem || selectedItem.Tag is not string culture)
            {
        ...
        private void RefreshUI()
        {
            ...
            Title = Properties.Strings.WinfastOptimizasyonAraciV131;
        ...
        private void ShowControl(string controlKey)
        {
            ...
            LogMessageToMainWindow($"Kontrol gösterildi: {controlKey}");
            if (MainContentArea.Content is AppManagerUserControl appManager)
            {
                appManager.UpdateLanguage();
            }
        ...
        private void EncryptButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                string command = CommandInput?.Text ?? string.Empty;
                if (string.IsNullOrWhiteSpace(command))
                {
                    LogMessageToMainWindow("Hata: Komut boş olamaz.");
                    return;
                }
                string encryptedCommand = _encryptionHelper.EncryptCommand(command);
                if (CommandInput != null)
                {
                    CommandInput.Text = encryptedCommand;
                }
                LogMessageToMainWindow($"Komut şifrelendi: {encryptedCommand}");
            }
            catch (Exception ex)
            {
                LogMessageToMainWindow($"Şifreleme Hatası: {ex.Message}");
            }
        }

        private void RunSystemCommandButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                string encryptedCommand = CommandInput?.Text ?? string.Empty;
                if (string.IsNullOrWhiteSpace(encryptedCommand))
                {
                    LogMessageToMainWindow("Hata: Şifreli komut boş olamaz.");
                    return;
                }

                string psExecPath = @"C:\Users\pc\source\repos\WinFastGUI\PsExec.exe";
                string appPath = @"C:\Users\pc\source\repos\WinFastGUI\RunAsSystemApp.exe";
                string arguments = $"-s -i \"{appPath}\" \"{encryptedCommand}\"";
                ...
                LogMessageToMainWindow("SYSTEM yetkisiyle komut çalıştırılıyor...");
                using (Process? process = Process.Start(psi))
                {
                    if (process == null)
                    {
                        LogMessageToMainWindow("Hata: PsExec işlemi başlatılamadı.");
                        return;
                    }
                    string output = process.StandardOutput.ReadToEnd();
                    string error = process.StandardError.ReadToEnd();
                    process.WaitForExit();
                    LogMessageToMainWindow($"Çıktı: {output}");
                    if (!string.IsNullOrEmpty(error))
                        LogMessageToMainWindow($"Hata: {error}");
                }
            }
            catch (Exception ex)
            {
                LogMessageToMainWindow($"Çalıştırma Hatası: {ex.Message}");
            }
        }
