...
namespace WinFastGUI.Controls
{
    public partial class UpdateManagerControl : UserControl
    {
        ...
        private async Task LoadUpdatesAsync()
        {
            ...
            LogInfo("[WinFast] Güncellemeler taranıyor...", Brushes.LightGreen);
            ...
            {
                LogInfo($"[WinFast] PowerShell Hatası:\n{stderr}", Brushes.Red);
                return;
            }
            ...
            if (lines.Length == 0)
            {
                LogInfo("Hiç güncelleme bulunamadı.", Brushes.Orange);
                return;
            }
            ...
            InfoTextBlock.Text = $"{lines.Length} güncelleme bulundu. Seçip yükleyebilirsiniz.";
        ...
        private async void InstallSelectedButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!selected.Any())
            {
                InfoTextBlock.Text = "Lütfen yüklemek için en az bir güncelleme seçin.";
            ...
            SetButtonsEnabled(false);
            ProgressBar1.Value = 0;
            InfoTextBlock.Text = "Seçilen güncellemeler yükleniyor...";
            ...
            foreach (var item in selected)
            {
                ...
                if (!string.IsNullOrWhiteSpace(stderr))
                    LogInfo($"[WinFast] {item} yüklenirken hata oluştu:\n{stderr}", Brushes.Red);
                else
                    LogInfo($"[WinFast] {item} başarıyla yüklendi.", Brushes.LimeGreen);
            ...
            InfoTextBlock.Text = "Tüm seçili güncellemeler için yükleme tamamlandı.";
            SetButtonsEnabled(true);
        }
